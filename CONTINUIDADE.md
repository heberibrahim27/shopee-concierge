...
- **Admin, próximos passos (documento do GPT 26/09, auditado)**: (1) fila de divulgação como tela — lista ranqueada de `deal_candidates` com preço atual, cupom aplicável, histórico, comissão informada, interesse recente (cliques/buscas), última divulgação e ações copiar link/texto/imagem + "marcar como divulgado" (precisa decidir como convive com os crons que já publicam sozinhos); (2) "Atualizado às" por fonte no cabeçalho (Shopee/Awin/Lomadee/link-check) em vez da hora do render; (3) pausar cupom e corrigir associação manualmente, com histórico e volta ao dado da fonte; (4) tirar o verde residual dos ícones/nav do admin; (5) conferir `/admin/cupons` renderizado com dado real (feito só com tsc).
- **Redes de cupom que dependem do Heber**: Rakuten Advertising (Coupon Feed API) e Admitad (API de cupons) — cadastro e aprovação por loja são manuais; só depois vale escrever o importador (mesmo molde do source-coupons). Sugestão do documento do GPT (26/09).
- **Voto de cupom**: `coupon_feedback` só coleta; quando houver volume, mostrar "N confirmaram" no card e bloco no /admin pra pausar cupom com muita falha.
- **Promoções Shopee (shopeeOfferV2) em produção**: rodar `/api/cron/source-shopee-offers?dry=1` com o CRON_SECRET e conferir os campos; se vierem certos, rodar sem `dry` e abrir `/cupom/shopee`. Ver FEITO.md 2026-09-26.
- **Alerta de preço em produção**: depois do deploy, criar um alerta com o próprio número e conferir boas-vindas + linha em `price_alerts` + resposta do cron (ver FEITO.md 2026-09-26). Ainda não aparece no /admin — contar com `select status, count(*) from price_alerts group by 1`.
- **Próximos de custo zero ainda não começados**: indexar feeds inteiros de Nike/Olympikus (Kabum já está); favoritos no servidor (hoje o alerta já captura contato, o favorito continua só no navegador); bloco de alertas no /admin.
